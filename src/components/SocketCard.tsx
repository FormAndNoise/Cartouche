  ...
  function SocketCard({ socket, onOpen }, ref) {
    ...
    const handleDrop = async (e: DragEvent) => {
      ...
      if (socket.locked) {
        board.pushToast(
          "error",
          `[LOCKED] Socket "${socket.title || socket.position}" is locked — unlock it to attach files.`,
        );
        return;
      }
        ...
        <div className="card-top">
          ...
          {socket.locked && (
            <span
              className="lock-icon"
              aria-label="Locked"
              title="Locked — destructive edits are disabled"
            >
              🔒
            </span>
          )}
        ...
        <div className="thumb">
          {isAttaching ? (
            ...
            winner.preview_state === "ready" && winner.preview_uri ? (
              ...
              <span
                className="doc-icon"
                aria-label={`${winner.media_kind} document`}
              >
