...
function err(code: string, message: string, details: Record<string, unknown> | null = null): ApiError {
...
async function sha256Hex(bytes: Uint8Array): Promise<string> {
  try {
    ...
    // Non-secure context fallback (not cryptographically meaningful).
    let h = 0;
...
const STATUS_VALUES = new Set(['not_started', 'in_progress', 'needs_review', 'done']);
...
export class MockBackendClient implements BackendClient {
  ...
  private getOrThrow(projectPath: string): ProjectState {
    ...
    if (!p) throw err('NOT_FOUND', `Project not found at ${projectPath}`, { project_path: projectPath });
    return p;
  }

  private socketOrThrow(p: ProjectState, socketId: string): SocketState {
    const s = p.sockets.find((x) => x.id === socketId);
    if (!s) throw err('SOCKET_NOT_FOUND', `Socket ${socketId} not found`, { socket_id: socketId });
    return s;
  }
  ...
  private toSocket(s: SocketState): Socket {
    return this.toProject({ name: '', path: '', grid_columns: 1, sockets: [s] }).sockets[0];
  }

  /* ---------------- projects ---------------- */

  createProject(req: { name: string; socket_count: number; project_path: string }): Promise<Project> {
    return this.ipc(() => {
      ...
      if (!Number.isInteger(req.socket_count) || req.socket_count <= 0) {
        throw err('INVALID_SOCKET_COUNT', 'socket_count must be a positive integer', {
      ...
      if (this.projects.has(req.project_path)) {
        throw err('PATH_NOT_WRITABLE', `A project already exists at ${req.project_path}`, {
  ...
  updateProject(req: { project_path: string; name?: string; grid_columns?: number }): Promise<Project> {
    return this.ipc(() => {
      const p = this.getOrThrow(req.project_path);
      if (req.grid_columns !== undefined) {
        if (!Number.isInteger(req.grid_columns) || req.grid_columns < 1 || req.grid_columns > 4) {
          throw err('VALIDATION_ERROR', 'grid_columns must be an integer between 1 and 4', {
      ...
      if (req.name !== undefined) {
        if (req.name.trim() === '') throw err('INVALID_NAME', 'Project name must not be empty');
  ...
  updateSocket(req: {
    ...
    return this.ipc(() => {
      ...
      const touchesContent = req.title !== undefined || req.notes !== undefined || req.metadata !== undefined;
      ...
      if (req.metadata !== undefined) {
        if (!STATUS_VALUES.has(req.metadata.status)) {
          throw err('VALIDATION_ERROR', `Invalid status value: ${req.metadata.status}`);
        }
  ...
  setSocketLock(req: { project_path: string; socket_id: string; locked: boolean }): Promise<Socket> {
    return this.ipc(() => {
      const p = this.getOrThrow(req.project_path);
      const s = this.socketOrThrow(p, req.socket_id);
      s.locked = req.locked;
  ...
  reorderSockets(req: { project_path: string; ordered_socket_ids: string[] }): Promise<Project> {
    return this.ipc(() => {
      const p = this.getOrThrow(req.project_path);
      const ids = req.ordered_socket_ids;
      if (new Set(ids).size !== ids.length) throw err('DUPLICATE_ID', 'Duplicate socket ids in reorder list');
      const currentIds = new Set(p.sockets.map((s) => s.id));
      for (const id of ids) {
        if (!currentIds.has(id)) throw err('MISSING_SOCKET', `Unknown socket id ${id}`, { socket_id: id });
      }
      if (ids.length !== p.sockets.length) {
        throw err('MISSING_SOCKET', 'Reorder list must cover every socket in the project');
      }
  ...
  selectWinner(req: { project_path: string; socket_id: string; work_id: string | null }): Promise<Socket> {
    return this.ipc(() => {
      const p = this.getOrThrow(req.project_path);
      const s = this.socketOrThrow(p, req.socket_id);
      if (s.locked) throw err('LOCKED', 'Socket is locked', { socket_id: s.id });
      if (req.work_id !== null && !s.works.some((w) => w.id === req.work_id)) {
        throw err('WORK_NOT_IN_SOCKET', `Work ${req.work_id} is not attached to socket ${s.id}`, {
          work_id: req.work_id,
          socket_id: s.id,
        });
      }
  ...
  /**
   * Browser-mode helper: register the Blob behind a path/URL created by the
   * drag-and-drop / file-picker layer so the mock can hash it, build a
   * preview, and extract text — mirroring what the Rust backend does with
  ...
  private async buildWork(s: SocketState, sourcePath: string): Promise<WorkState> {
    ...
    const work: WorkState = {
      ...
      extracted_text_state: TEXT_EXTRACT_EXTS.has(ext(sourcePath)) ? 'pending' : 'unsupported',
      metadata_json: '{}',
      source_blob: blob,
    };
    s.works.push(work);

    // Simulate the background preview job (US-B07).
    if (kind === 'image') {
      setTimeout(() => {
    ...
    // Simulate the background extraction job (US-B06).
  ...
  attachWork(req: { project_path: string; socket_id: string; source_path: string }): Promise<Socket> {
    return this.ipc(async () => {
      const p = this.getOrThrow(req.project_path);
      const s = this.socketOrThrow(p, req.socket_id);
      if (s.locked) throw err('LOCKED', 'Socket is locked', { socket_id: s.id });
      if (!req.source_path) throw err('FILE_UNREADABLE', 'source_path is required');
  ...
  async importDroppedFiles(req: {
    ...
    if (s.locked) {
      return {
        accepted: [],
        rejected: req.paths.map((path) => ({ path, reason: 'Socket is locked', code: 'LOCKED' })),
      };
    }
    const accepted: Work[] = [];
  ...
  removeWork(req: {
    ...
    return this.ipc(() => {
      ...
      if (!w) throw err('NOT_FOUND', `Work ${req.work_id} not found`, { work_id: req.work_id });
      if (s.selected_work_id === w.id && !req.force) {
        throw err('IS_SELECTED', 'This work is the selected winner. Confirm removal to proceed.', {
          work_id: w.id,
        });
      }
  ...
  refreshExtractedText(req: { project_path: string; socket_id: string; work_id: string }): Promise<Socket> {
    return this.ipc(() => {
      const p = this.getOrThrow(req.project_path);
      const s = this.socketOrThrow(p, req.socket_id);
      const w = s.works.find((x) => x.id === req.work_id);
      ...
      if (!TEXT_EXTRACT_EXTS.has(ext(w.title))) {
        throw err('UNSUPPORTED_FORMAT', `No text extractor for ${w.media_kind}`, { work_id: w.id });
      }
  ...
  private parseImportCsv(csvText: string): { headers: string[]; rows: string[][] } {
    const table = parseCsv(csvText);
    if (table.length === 0) throw err('CSV_PARSE_ERROR', 'CSV file is empty');
    const headers = table[0].map((h) => h.trim().toLowerCase());
    if (!headers.includes('title')) {
      throw err('MISSING_REQUIRED_COLUMN', "CSV must contain a 'title' column", { headers });
    }
  ...
  importCsv(req: {
    ...
    return this.ipc(() => {
      ...
      job.timer = setInterval(() => {
        ...
        job.status.progress = rows.length === 0 ? 100 : Math.round((i / rows.length) * 100);
        job.status.warnings = [...warnings];
        if (i >= rows.length) {
          if (job.timer) clearInterval(job.timer);
          job.status.state = 'done';
          job.status.progress = 100;
          job.status.result = { rows_total: rows.length, rows_processed: processed, rows_skipped: skipped };
        }
      }, this.jobTickMs);
  ...
  getJob(req: { project_path: string; job_id: string }): Promise<JobStatus> {
    return this.ipc(() => {
      ...
      if (!job) throw err('NOT_FOUND', `Job ${req.job_id} not found`, { job_id: req.job_id });
  ...
  exportProject(req: {
    ...
    return this.ipc(() => {
      this.getOrThrow(req.project_path);
      if (!req.destination_path) throw err('PATH_NOT_WRITABLE', 'destination_path is required');
      return { path: req.destination_path, manifest_sha256: 'mock-manifest-hash' };
    });
  }
  ...
  /* ---------------- demo seeding (browser mode only) ---------------- */

  /** Creates a sample tarot project with a few pre-filled sockets. */
  async seedDemoProject(path = 'C:/Users/artist/Projects/tarot-deck'): Promise<Project> {
    ...
    await this.updateSocket({
      ...
      metadata: { status: 'in_progress', medium: 'digital painting', tags: 'major-arcana, opener', due_date: null },
    });
    await this.updateSocket({
      project_path: path,
      socket_id: project.sockets[1].id,
      title: 'I — The Magician',
      metadata: { status: 'needs_review', medium: 'ink', tags: 'major-arcana', due_date: '2026-09-01' },
    });
    const s2 = project.sockets[2];
    await this.updateSocket({ project_path: path, socket_id: s2.id, title: 'II — The High Priestess (locked)' });
    await this.setSocketLock({ project_path: path, socket_id: s2.id, locked: true });
