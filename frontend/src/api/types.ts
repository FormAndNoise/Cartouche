 ...
 * even when the current Rust backend uses integers internally.
 */

export type MediaKind = 'image' | 'pdf' | 'docx' | 'text' | 'other';

export type SocketStatus = 'not_started' | 'in_progress' | 'needs_review' | 'done';

/**
 * Fixed metadata schema (REQUIREMENTS.md §6.3 [ASSUMPTION]).
 * The editor renders exactly these fields; `metadata_json` in the DB
 * already supports arbitrary keys for future extension.
 */
